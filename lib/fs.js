...
export async function getFilesFromFolder(dir) {
...
export const getVercelOutput = (root) => new URL('./.vercel/output/', root);
 ...
 */
export async function copyFilesToFunction(files, outDir, exclude = []) {
    ...
    for (const origin of fileList) {
        ...
        if (isSymlink) {
            ...
            await fs.symlink(nodePath.relative(fileURLToPath(new URL('.', dest)), realdest), dest, isDir ? 'dir' : 'file');
        }
        else if (!isDir) {
